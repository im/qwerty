...
arr.forEach(item => {
    item.id = path.basename(item.url, '.json')
    delete item.url
    const data = fs.readFileSync(path.resolve(dir, item.id + '.json'))
    const arr = JSON.parse(data.toString())
    console.log('item.id: ', item.id, item.name, arr.length)
    item.length = arr.length
})

fs.writeFileSync('./src/dict.ts', `export default ${JSON.stringify(arr)}` )