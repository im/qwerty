...
test.describe("Dictionary manage", () => {
  ...
  test("Switch category", async ({ page }) => {
    ...
    await expect(
      await page
        .getByRole("radio", { name: /^考研$/ })
        .getAttribute("aria-checked")
    ).toBeTruthy();
    await expect(
      await page.getByRole("button", { name: /考研/g }).first().isVisible()
    ).toBeTruthy();
    ...
    await expect(
      await page.getByRole("button", { name: /GRE/g }).first().isVisible()
    ).toBeTruthy();
  });
  ...
  test("Close dictionary settings", async ({ page }) => {
    ...
    await page.waitForURL("**/");
    await expect(
      await page.getByText("Start").first().isVisible()
    ).toBeTruthy();
  });
